...
void SDLGraphicsProgram::Loop(){
    ...
    // Create our terrain
    Terrain* myTerrain = new Terrain(512,512,"terrain.ppm");
    myTerrain->LoadTexture("terrainMap.ppm", 255);
    Terrain* myOcean = new Terrain(512, 512, "ocean.ppm");
    myOcean->LoadTexture("oceanMap.ppm", 127);
    // Create a node for our terrain 
    SceneNode* terrainNode;
    SceneNode* oceanNode;
    terrainNode = new SceneNode(myTerrain);
    oceanNode = new SceneNode(myOcean);
    // Set our SceneTree up
    m_renderer->setRoot(terrainNode);
    terrainNode->AddChild(oceanNode);

    // Set a default position for our camera
    m_renderer->GetCamera(0)->SetCameraEyePosition(125.0f,200.0f,500.0f);
