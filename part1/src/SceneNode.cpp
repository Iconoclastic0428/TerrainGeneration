...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera, float theta){
    if(m_object!=nullptr){
        // TODO: Implement here!
	...
	m_shader.SetUniform1f("theta", theta);
	...
	m_shader.SetUniform3f("viewPos",
		...
		for(int i =0; i < m_children.size(); ++i){
			m_children[i]->Update(projectionMatrix, camera, theta);
		}
	}
}
