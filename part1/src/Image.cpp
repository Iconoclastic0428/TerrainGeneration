...
void Image::LoadPPM(bool flip, int val){
  ...
  if (ppmFile.is_open()){
      ...
      while ( getline (ppmFile,line) ){
         ...
         if(line[0]=='P'){
            ...
            if(m_width > 0 && m_height > 0){
                m_pixelData = new uint8_t[m_width*m_height*4];
                if(m_pixelData==NULL){
    ...
    if(flip){
        ...
        uint8_t* copyData = new uint8_t[m_width*m_height*4];
        for(int i =0; i < m_width*m_height*4; ++i){
            copyData[i]=m_pixelData[i];
        }
        //memcpy(copyData,m_pixelData,(m_width*m_height*3)*sizeof(uint8_t));
        unsigned int pos = (m_width*m_height*4)-1;
        for(int i =0; i < m_width*m_height*4; i+=4){
            m_pixelData[pos]=copyData[i+3];
            m_pixelData[pos-1]=copyData[i+2];
            m_pixelData[pos-2]=copyData[i+1];
	    m_pixelData[pos-3]=copyData[i];
            pos-=4;
        }
        delete[] copyData;
    }
...
(int)color[x*y+2] << ")";*/
    m_pixelData[(x*4)+m_height*(y*4)] = r;
    m_pixelData[(x*4)+m_height*(y*4)+1] = g;
    m_pixelData[(x*4)+m_height*(y*4)+2] = b;
